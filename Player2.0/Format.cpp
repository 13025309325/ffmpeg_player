...
int init_audio(AVCodecContext * codec_ctx)
{
	...
	au_convert_ctx = swr_alloc_set_opts(au_convert_ctx, AV_CH_LAYOUT_STEREO, AV_SAMPLE_FMT_S16, 44100, in_channel_layout, codec_ctx->sample_fmt, codec_ctx->sample_rate, 0, NULL);
 ...
 int decode1(AVFormatContext * fmt_ctx, AVCodecContext *dec_ctx, AVFrame *frame, AVPacket *pkt, Format *fmt)
{
	...
	while (ret >= 0) {
		...
		if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
			...
			fprintf(stderr, "Error during1 decoding\n");
			exit(1);
		}
...
int decode2(AVFormatContext * fmt_ctx, AVCodecContext *dec_ctx, AVFrame *frame, AVPacket *pkt, Format *fmt)
{
	...
	while (ret >= 0) {
		...
		if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
			...
			fprintf(stderr, "Error during2 decoding\n");
			exit(1);
		}
